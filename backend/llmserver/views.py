...
def getPageMain(request):
    ...
    question1_option1 = f"""
            ...
            1.要求符合日常用户登录界面业务逻辑.
            2.组件基于element-ui框架.
            3.根容器背景使用指定图片URL：{WEB_URL}/{username}/{safe_datetime}/background.jpg，请确保图片可以显示,正确保留url中的空格，使用行内式background-image: url('{WEB_URL}/{username}/{safe_datetime}/background.jpg')
    ...
    question1_option2 = f"""
        ...
        3. 整个页面作为背景，宽高 100%（width: 100%; height: 100vh;），使用 background-size: cover 显示图片：url('{WEB_URL}/{username}/{safe_datetime}/background.jpg')，居中对齐但在右侧留出一块区域放置登录卡片（margin-right: 80px）。
...
def getPageVice(request):
    ...
    question1_option1 = f"""
        ...
        1.要求符合日常用户注册界面业务逻辑.
        2.组件基于element-ui框架.
        3.根容器背景使用指定图片URL：{WEB_URL}/{username}/{safe_datetime}/background.jpg，请确保图片可以显示,正确保留url中的空格，注意图片需要完整显示background-size: cover，使用行内式background-image: url('{WEB_URL}/{username}/{safe_datetime}/background.jpg')
        4.卡片容器需要始终居中,样式要求如下width: 20%;position: absolute;top: 50%;left: 50%;transform: translate(-50%, -50%);
        5.登录卡片内部划分为标题区、输入区、操作按钮区三个纵向模块，各模块间距比例为1:2:1。
        6.标题区域固定于卡片顶部，显示欢迎登录等字样，使用大字号粗体文本居中显示，下方保留与输入区的动态间距。
    ...
    2. 所有样式均使用行内式写在元素的 style 属性中，不能使用 <style> 或 scoped 样式。
    3. 页面背景使用图片：url('{WEB_URL}/{username}/{safe_datetime}/background.jpg')，要求覆盖全屏 (width: 100%; height: 100vh; background-size: cover;)，且在右侧留出一块区域放置注册卡片 (margin-right: 80px)。
